...
plot_data <- data.frame(
  ...
  `Mdelta-fixed` = c(750, 3263, 13004, 542), # TODO: replace with your data
  check.names = FALSE
)

method_labels <- c("Mdelta", "Mdelta-fixed")
method_colors <- c(
  "Mdelta" = "#A61D24",
  "Mdelta-fixed" = "#78B0B8"
)
...
create_three_method_barplot <- function(
    ...
    group_col = "group",
    method_labels = c("Mdelta", "Mdelta-fixed"),
    fill_colors = c("Mdelta" = "#A61D24", "Mdelta-fixed" = "#78B0B8"),
    x_label = "",
    y_label = "Value",
    axis_text_size = 32,
    axis_title_size = 32,
...
cairo_pdf(
  "E:/Users/drinkwater/Desktop/R-script-template/Paper/hotstorage26/evaluation/compression/overall/overall.pdf",
  width = 12,
  height = 5
)
print(p)
