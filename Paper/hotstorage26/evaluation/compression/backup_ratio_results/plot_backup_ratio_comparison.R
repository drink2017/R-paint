...
read_method_pair <- function(dataset_name) {
  ...
  data.frame(
    ...
    design3 = design_df$compression_ratio,
    odess = odess_df$compression_ratio
  )
}
...
plot_one_dataset <- function(dataset_name, export_name) {
  ...
  p <- ggplot(long_df, aes(x = backup_index, y = ratio, color = method)) +
    geom_line(size = 2.8) +
    scale_color_manual(
...
plot_one_dataset("linux", "backup_ratio_linux.pdf")
plot_one_dataset("docker_gitlab", "backup_ratio_docker_gitlab.pdf")
plot_one_dataset("logs", "backup_ratio_logs.pdf")
plot_one_dataset("WEB", "backup_ratio_WEB.pdf")
